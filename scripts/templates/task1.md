...
area: benchmark
priority: 100
verify: 'cd __TASKDIR__/RecurKit && swift run --disable-sandbox RecurKitChecks'
...
---
## Goal
Fix the bug behind the user report in `__TASKDIR__/RecurKit/PLAN.md`, and
...
`__TASKDIR__/RecurKit`. A user reports weekly/daily series drifting by an
...
rules: `__TASKDIR__/RecurKit/PLAN.md`.

## Do
Read `__TASKDIR__/RecurKit/PLAN.md`. Write a deterministic check that
...
## Do not
Do not touch anything outside `__TASKDIR__/`. Do not special-case the
