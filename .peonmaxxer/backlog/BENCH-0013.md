...
area: benchmark
priority: 120
verify: 'export PATH="/opt/homebrew/bin:$PATH" && cd src/opencode-muse-spark-1.2-contributor-free/r1/task3/pricing-site && npm ci && npm test && npm run build'
...
`src/opencode-muse-spark-1.2-contributor-free/r1/task3/pricing-site/PLAN.md`: hero, data-driven feature grid, and
an interactive pricing calculator, in Svelte 5 + Vite.

## Context
The scaffold is in `src/opencode-muse-spark-1.2-contributor-free/r1/task3/pricing-site`: pinned `package.json` +
...
## Do
Read `src/opencode-muse-spark-1.2-contributor-free/r1/task3/pricing-site/PLAN.md`. Implement
...
`npm ci`. Do not touch anything outside `src/opencode-muse-spark-1.2-contributor-free/r1/task3/`.
